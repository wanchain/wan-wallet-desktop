...
@observer
class BNBNormalTransForm extends Component {
  state = {
    gasFee: 0,
    advanced: false,
  ...
  updateGasLimit = () => {
    ...
    try {
      tx = { from, to, data };
      wand.request('transaction_estimateGas', { chainType: 'BNB', tx }, (err, gasLimit) => {
        if (err) {
  ...
  render() {
    ...
    return (
      <div>
                ...
                advanced
                  ...
                  : <Form.Item label={intl.get('NormalTransForm.fee')}>
                    {getFieldDecorator('fee', { rules: [{ required: true, message: intl.get('NormalTransForm.pleaseSelectTransactionFee') }] })(
                      <Radio.Group>
                        <Radio.Button onClick={e => this.handleClick(e, minGasPrice, gasLimit, nonce, minFee)} value="minFee"><p>{intl.get('NormalTransForm.slow')}</p>{minFee} BNB</Radio.Button>
                        <Radio.Button onClick={e => this.handleClick(e, averageGasPrice, gasLimit, nonce, averageFee)} value="averageFee"><p>{intl.get('NormalTransForm.average')}</p>{averageFee} BNB</Radio.Button>
                        <Radio.Button onClick={e => this.handleClick(e, maxGasPrice, gasLimit, nonce, maxFee)} value="maxFee"><p>{intl.get('NormalTransForm.fast')}</p>{maxFee} BNB</Radio.Button>
                      </Radio.Group>
                    )}
                  </Form.Item>
              }
        ...
        <AdvancedOption visible={advancedVisible} onCancel={this.handleAdvancedCancel} onSave={this.handleSave} estimateGas={this.estimateGasInAdvancedOptionForm} from={from} chain='BNB' />
        {
          confirmVisible &&
...
export default BNBNormalTransForm;
