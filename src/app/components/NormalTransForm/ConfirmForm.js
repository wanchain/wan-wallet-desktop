...

          {
            isPrivate ? (
              <Table className={style.splitAmountTable} scroll={{ x: 480 }} rowKey={'face'} dataSource={this.state.dataSource} columns={this.columns()} pagination={false} />
                ...
                <div>
                  <Form.Item label={intl.get('NormalTransForm.ConfirmForm.gasPrice') + ' (' + (chain === 'ETH' || chain === 'BNB' ? 'Gwei' : ((chain === undefined || chain === 'WAN') ? intl.get('AdvancedOptionForm.gwin') : chain)) + ')'}> {
                    getFieldDecorator(
