...
import { BSCPATH, WALLETID } from 'utils/settings';
...
class BnbAddress {
  ...
  @action addAddress(newAddr) {
    self.addrInfo['normal'][newAddr.address] = {
      name: newAddr.name ? newAddr.name : `BSC-Account${Number(newAddr.start) + 1}`,
    ...
    addrArr.forEach(addr => {
      if (!Object.keys(self.addrInfo[type] || []).includes(addr.address)) {
        if (addr.name === undefined) {
          addr.name = `BSC-Account${parseInt(((/[0-9]+$/)).exec(addr.path)[0]) + 1}`;
        }
  ...
  @action updateTransHistory(initialize = false) {
    ...
    wand.request('transaction_showRecords', (err, val) => {
      if (!err && val.length !== 0) {
        let tmpTransHistory = {};
        val = val.filter(item => item.chainType === CHAIN);
        val.forEach(item => {
  ...
  @computed get getNormalAddrList() {
    ...
    normalArr.forEach(item => {
      ...
      addrList.push({
        ...
        path: `${BSCPATH}${self.addrInfo[type][item].path}`,
        action: 'send',
        wid: WALLETID.NATIVE
      });
    });
